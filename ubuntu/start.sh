...
# do
    ...
    # tech=zip
    # key=guide_test-zip
    mkdir -p resultados/resultados-$tech
    ...
    strace -c -e trace=read,write,open zip -v $arquivo_comprimido $arquivo_original > /dev/null 2> resultados/resultados-$tech/resultado-$key-$round.txt
    ...
    echo "" >> resultados/resultados-$tech/resultado-$key-$round.txt
    echo "original reduzido taxa" >> resultados/resultados-$tech/resultado-$key-$round.txt
    echo "$tamanho_original $tamanho_comprimido $taxa_percentual" >> resultados/resultados-$tech/resultado-$key-$round.txt
